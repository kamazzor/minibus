...
//		Config config = ConfigUtils.loadConfig( args[0], new PConfigGroup() ) ;
		...
		Config config = ConfigUtils.loadConfig("C:\\JavaProjects\\matsim-libs-master\\contribs\\minibus\\config.xml", new PConfigGroup() ) ;
		File file = new File ("C:\\JavaProjects\\matsim-libs-master\\contribs\\minibus\\config.xml");

		if(file.length() <= 0){
			/////////////////////////////////////////
			log.info("Arg 1: config.xml is locked or missing.");
			log.info("Check http://svn.vsp.tu-berlin.de/repos/public-svn/matsim/scenarios/countries/atlantis/minibus/ for an example.");
			System.exit(1);
		}
